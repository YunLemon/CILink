class RepositoriesController < ApplicationController
  def index    
    ...
    if @search_text
      @repo = TravisJavaRepository.where("repo_name LIKE ? AND id >= ? AND builds > ? AND stars>?", '%' + @search_text + '%', 1,  @builds.to_i, @stars.to_i).paginate(:page => params[:page], :per_page => 30).order('id')
    else
